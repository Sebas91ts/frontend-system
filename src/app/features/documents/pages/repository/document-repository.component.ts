...
export class DocumentRepositoryComponent implements OnInit {
  ...
  protected loadAreaOptions(): void {
    if (!this.isAdmin()) {
      this.areaOptions = this.authService.currentUser()?.tenantId ? [this.authService.currentUser()!.tenantId!] : [];
  ...
  protected roleScopeLabel(): string {
    ...
    return `Funcionario - area ${this.currentTenantLabel()}`;
  }

  protected currentTenantLabel(): string {
    return this.authService.currentUser()?.tenantId || this.documents[0]?.tenantId || this.selectedDocument?.tenantId || 'actual';
  }

  protected areaLabel(tenantId?: string): string {
    return tenantId || 'Sin area';
  }
  ...
  protected folderName(folderId?: string): string {
    ...
    return this.findFolderById(folderId, this.folderTree)?.name ?? 'Carpeta';
  }
  ...
  protected isEditable(document: DocumentMetadata): boolean {
    ...
    return document.editable !== false && this.isOfficeDocument(document);
  }
